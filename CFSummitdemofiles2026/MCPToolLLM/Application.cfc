component {
    ...
    this.name = "CFSummit2026MCPToolLLM";
    this.applicationTimeout = createTimeSpan(0, 4, 0, 0);
    this.sessionManagement = true;
    this.sessionTimeout = createTimeSpan(0, 0, 10, 0);
    this.setClientCookies = true;
    this.mappings["/mcpinventory"] = variables.appRoot & "components";
    ...
    public void function onRequestStart(required string targetPage) {
        ...
        if (
            ...
            !structKeyExists(application, "inventoryDatasource") ||
            !structKeyExists(application, "mcpToolProvider") ||
            application.mcpToolProvider != "mcpinventory.InventoryDatabaseToolV3"
        ) {
            initializeDemoState();
        }
    ...
    private void function initializeDemoState() {
        ...
        application.keystoreKeyIds = {
            openAi: "openaiapi_inventoryai",
            anthropic: "anthropicapi_inventoryai"
        };
        ...
        application.mcpToolProvider = "mcpinventory.InventoryDatabaseToolV3";
        application.mcpEndpoint = "http://localhost:8500/CFSummit2026/demos/MCPToolLLM/mcp/server.cfm";
        application.mcpNativeServerAvailable = false;
        application.mcpNativeClientAvailable = false;
        application.mcpTransportMode = "native-coldfusion-mcp-required";
        application.mcpServerError = "";
        application.cairoiConfig = {
            appId: "inventory-ai",
            environment: "conference",
            collectorUrl: "https://adobecoldfusion.com/cairoi/api/telemetry.cfm",
            apiKey: "cairoi-inventory-dev-key",
            failSilently: true,
            debug: true,
            asyncTelemetry: true,
        ...
        application.selectedAiModelKey = "openai-nano";
        application.openAiApiKey = getConfiguredOpenAiApiKey();
        application.anthropicApiKey = getConfiguredAnthropicApiKey();
        application.aiModelOptions = buildAiModelOptions();

        application.chatConfig = duplicate(application.aiModelOptions[application.selectedAiModelKey].chatConfig);

        initializeMcpServer();
        refreshAiConfig();
    }

    private void function initializeMcpServer() {
        try {
            configData = {
                serverInfo: {
                    name: "cf-inventory-database-mcp",
                    version: "2.2.0"
                },
                capabilities: {
                    tools: true,
                ...
                tools: [
                    { cfc: application.mcpToolProvider }
                ]
            };

            application.mcpServer = MCPServer(configData);
            application.mcpNativeServerAvailable = true;
        } catch (any error) {
            application.mcpServer = "";
            application.mcpNativeServerAvailable = false;
            application.mcpServerError = error.message;
        }
    }
    ...
    private void function refreshAiConfig() {
        ...
        if (structKeyExists(application, "chatConfig")) {
            selectedKey = structKeyExists(application, "selectedAiModelKey") ? application.selectedAiModelKey : "openai-nano";
            ...
            application.chatConfig = duplicate(application.aiModelOptions[selectedKey].chatConfig);
        }
    }

    private string function getConfiguredAnthropicApiKey() {
        return readKeystoreApiKey(application.keystoreKeyIds.anthropic);
    }
    ...
    private string function cleanApiKey(required string keyValue) {
        cleaned = trim(replace(arguments.keyValue, chr(65279), "", "all"));
    ...
    private boolean function isUsableOpenAiKey(required string keyValue) {
        return len(trim(arguments.keyValue)) > 20;
    }

    private boolean function isUsableAnthropicKey(required string keyValue) {
        cleaned = trim(arguments.keyValue);
        return len(cleaned) > 40 && left(cleaned, 7) == "sk-ant-";
    }

    private struct function buildAiModelOptions() {
        options = {};
        options["openai-nano"] = {
            key: "openai-nano",
            label: "ChatGPT Nano",
            ...
            available: isUsableOpenAiKey(application.openAiApiKey),
            chatConfig: {
                provider: "openAi",
        ...
        options["claude-haiku"] = {
            ...
            available: isUsableAnthropicKey(application.anthropicApiKey),
            chatConfig: {
                provider: "anthropic",
                apiKey: application.anthropicApiKey,
